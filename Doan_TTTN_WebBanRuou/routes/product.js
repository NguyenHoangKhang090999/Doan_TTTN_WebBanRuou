...
router.get("/:name", async function (req, res) {
  ...
  for (itemPro of listPro) {
    let newNamePro = itemPro.TENDONG;
    breadcrumb = newNamePro;
    newNamePro = replaceNameProduct(itemPro.TENDONG).toLowerCase();
  ...
  let listComment = await modelProduct.getComment(newItemPro.MADONG);

  res.render("site/product-detail.ejs", {
    itemPro: newItemPro,
    listCat: listCat,
    listPro: listPro,
...
// API
router.get("/api/all-product", async function (req, res) {
  let data = await modelProduct.list();
  res.json(data);
});
...
router.get("/api/khuyen-mai", async function (req, res) {
  let data = await modelProduct.listkm();
  res.json(data);
});

router.get("/api/listproductofuser/:id", async function (req, res) {
  let iduser = req.params.id;
  let data = await modelProduct.listProductofUser(iduser);
  res.json(data);
});
